  ...
  if (loading) {
    return (
      <div className="flex-1 px-4 py-8 max-w-lg mx-auto w-full animate-page-enter">
        <div className="h-8 w-40 mx-auto mb-6 rounded bg-[var(--color-bg-tertiary)] animate-pulse" />
        ...
        <div className="h-6 w-32 mb-4 rounded bg-[var(--color-bg-tertiary)] animate-pulse" />
        {[...Array(3)].map((_, i) => (
          <div
            key={i}
            className="h-12 mb-2 rounded glass border border-[var(--color-bg-tertiary)] animate-pulse"
          />
        ))}
      </div>
    );
